...
export async function createInvoice(input) {
  ...
  const labourCharge =
    input?.labourCharge !== undefined && input?.labourCharge !== null && input?.labourCharge !== ""
      ? parseMoney(input.labourCharge, "Labour charge", { min: 0 })
      : "0.00";
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const billedBy = await tx.user.findUnique({ where: { id: billedById }, select: { id: true, name: true } });

    if (!billedBy) {
      throw new ApiError("Billed-by user not found.", 400);
    }
    ...
    const profitAmount = preparedItems
      ...
      .toFixed(2);
    const totalAmount = (Number(subtotal) + Number(gstAmount) + Number(labourCharge)).toFixed(2);
    const finalProfitAmount = (Number(profitAmount) + Number(labourCharge)).toFixed(2);
